...
def myPlot(dataList, boxList):
    x = list(map(lambda item:item[0], dataList))
    y = list(map(lambda item:item[1], dataList))
    plt.scatter(x, y)
...
training_file, testing_file = "data/fuzzyART/TrainingData.txt", "data/fuzzyART/TestingData.txt"
...
while True:
    ...
    for i in range(len(trainingData)):
        ...
        while len(tjList) != 0 :
            ...
            if sum(fuzzyAnd(trainingData[i], categoryList[index]))/sum(trainingData[i]) >= rho :
                categoryList[index] = list(map(lambda x, y: x*beta + y*(1-beta),
    ...
    myPlot(trainingData, categoryList[-5:])
